  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="rounded-full"><Upload className="mr-2 h-4 w-4" /> Upload resume</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Upload your resume</DialogTitle>
          <DialogDescription>PDF, DOCX, TXT or image. We'll parse it into editable sections and score it for ATS.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Title (optional)</label>
            <Input className="mt-1" placeholder="e.g. Backend Internship Resume" value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>
          <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border/70 p-8 text-center text-sm hover:bg-accent/30">
            ...
            {file ? <span className="font-medium">{file.name}</span> : <><span>Click to choose a file</span><span className="text-xs text-muted-foreground">PDF · DOCX · TXT · PNG · JPG (max {MAX_MB}MB)</span></>}
          ...
          <Button onClick={handle} disabled={busy || !file} className="w-full rounded-full">
            {busy ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Parsing with AI…</> : "Parse & analyze"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
