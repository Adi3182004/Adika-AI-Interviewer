...
export function CreatorShowcase() {
  ...
  return (
    <section className="relative z-10 mx-auto max-w-6xl px-6 py-20">
      ...
      <h2 className="mt-3 font-display text-3xl md:text-4xl">A philosophy, in two frames.</h2>

      <div className="mt-12 grid gap-8 md:grid-cols-2">
        {/* Frame 1 — Never More to Have */}
        <div ref={a.ref} className={`relative overflow-hidden rounded-3xl bg-[#0a0816] p-8 md:p-10 ${slide(a.shown)}`}>
          <div className="grid items-center gap-6 md:grid-cols-[1fr_1.1fr]">
            <div className="text-white/80">
              ...
              <a href="#features" className="mt-5 inline-block border-b border-white/40 pb-0.5 text-xs uppercase tracking-widest text-white hover:border-white">
                Read more
              </a>
            </div>
        ...
        {/* Frame 2 — Never More to Create */}
        <div ref={b.ref} className={`relative overflow-hidden rounded-3xl bg-[#0a0816] p-8 md:p-10 ${slide(b.shown)}`} style={{ transitionDelay: "120ms" }}>
          <div className="grid items-center gap-6 md:grid-cols-[1fr_1.1fr]">
            <div className="text-white/80">
              <p className="text-[11px] uppercase tracking-[0.3em] text-white/40">Manifesto · 02</p>
              <p className="mt-4 text-sm leading-relaxed text-white/70">{SHORT_DESC}</p>
              <a href="#how" className="mt-5 inline-block border-b border-white/40 pb-0.5 text-xs uppercase tracking-widest text-white hover:border-white">
                Read more
              </a>
            </div>
            <div className="relative flex h-72 items-end justify-center md:h-96">
              <div
                ...
                className="absolute left-1/2 top-1/2 h-56 w-56 -translate-x-1/2 -translate-y-1/2 rounded-full md:h-72 md:w-72"
                style={{
                  background: "radial-gradient(circle at 50% 50%, #f4eeff 0%, #cdb6ff 55%, #9d76ff 100%)",
                }}
              />
              <img
                src={portrait.url}
                ...
                className="relative z-10 h-full w-auto max-w-full object-contain object-bottom"
              />
              <p className="absolute right-4 top-6 z-20 max-w-[60%] text-right font-display text-3xl leading-[0.95] text-white md:text-5xl">
                Never<br />More<br />to <em className="not-italic text-gold">Create</em>.
              </p>
            </div>
          </div>
        </div>
