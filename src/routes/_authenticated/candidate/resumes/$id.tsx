  ...
  async function improveSummary() {
    ...
    try {
      const { improved } = await improve({
        data: { section: "Professional Summary", current: content.summary },
      });
        ...
        <div className="space-y-6">
          ...
          >
            <Textarea
              ...
              placeholder="2-3 sentences positioning you."
            />
          </Section>
          ...
          <Section title="Skills (comma-separated)">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {content.skills.map((s, i) => (
                <Badge key={`${s}-${i}`} variant="secondary" className="rounded-full">
                  {s}
                </Badge>
              ))}
            </div>
                ...
                <div key={i} className="rounded-xl border border-border/60 p-4 space-y-2">
                  <div className="grid gap-2 md:grid-cols-3">
                    <Input
                      ...
                      value={e.company}
                      onChange={(ev) =>
                        updateArray(setContent, content, "experience", i, {
                    ...
                    <Input
                      ...
                      value={e.role}
                      onChange={(ev) =>
                        updateArray(setContent, content, "experience", i, { role: ev.target.value })
                      }
                    />
                    <Input
          ...
          >
            <div className="space-y-2">
              {content.education.map((e, i) => (
                <div
                  key={i}
                  className="grid gap-2 md:grid-cols-3 rounded-xl border border-border/60 p-3"
                >
                  <Input
                    placeholder="School"
                    value={e.school}
                    onChange={(ev) =>
                      updateArray(setContent, content, "education", i, { school: ev.target.value })
                    }
                  />
                  <Input
                    placeholder="Degree"
                    value={e.degree}
                    onChange={(ev) =>
                      updateArray(setContent, content, "education", i, { degree: ev.target.value })
                    }
                  />
                  <Input
                    placeholder="Year"
                    value={e.year}
                    onChange={(ev) =>
                      updateArray(setContent, content, "education", i, { year: ev.target.value })
                    }
                  />
                </div>
              ))}
            ...
            action={
              <Button
                ...
                onClick={() =>
                  setContent({
                    ...content,
                    projects: [...content.projects, { name: "", description: "" }],
                  })
                }
              >
                + Add
              ...
              {content.projects.map((p, i) => (
                <div key={i} className="rounded-xl border border-border/60 p-3 space-y-2">
                  <Input
                    ...
                    value={p.name}
                    onChange={(ev) =>
                      updateArray(setContent, content, "projects", i, { name: ev.target.value })
                    }
                  />
                  <Textarea
                    rows={2}
                    ...
                    value={p.description}
                    onChange={(ev) =>
                      updateArray(setContent, content, "projects", i, {
              ...
              <>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {resume.ats_score >= 80
                    ...
                    : resume.ats_score >= 60
                      ? "Solid baseline — apply the tips below."
                      : "Needs work — follow the section tips."}
                </p>
              </>
            ) : (
              <>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Click ATS Analysis to score this resume.
                </p>
              </>
            )}
          </div>
          {resume?.ats_feedback ? (
            <div className="glass rounded-2xl p-6">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {(resume.ats_feedback as any)?.summary}
              </p>
            ...
            <div className="glass rounded-2xl p-6 text-sm text-muted-foreground">
              Run ATS Analysis to get per-section feedback.
            </div>
          )}
        </aside>
      </div>
      ...
      <div className="glass mt-8 rounded-2xl p-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          ...
          <div>
            ...
            <Input
              className="mt-1"
              placeholder="Student / Intern / 0-1 yr / 2-4 yr"
              value={roleForm.experienceLevel}
              onChange={(e) => setRoleForm({ ...roleForm, experienceLevel: e.target.value })}
            />
          </div>
          <div className="md:col-span-2">
            ...
            <Textarea
              rows={4}
              className="mt-1"
              placeholder="Paste the JD to get the most precise analysis"
              value={roleForm.jobDescription}
              onChange={(e) => setRoleForm({ ...roleForm, jobDescription: e.target.value })}
            />
          </div>
        </div>
        ...
        {resume?.targeted_feedback ? (
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            <div className="glass rounded-2xl border border-primary/30 p-5">
              ...
              <p className="mt-1 font-display text-5xl text-primary">
                {(resume.targeted_feedback as any).role_fit_score}
              </p>
              <p className="mt-2 text-sm">{(resume.targeted_feedback as any).verdict}</p>
              <p className="mt-3 rounded-lg bg-secondary/50 p-3 text-xs">
                <span className="font-medium">Tailored summary:</span>{" "}
                {(resume.targeted_feedback as any).tailored_summary}
              </p>
            </div>
            <div className="glass rounded-2xl p-5">
              <p className="mb-3 flex items-center gap-2 text-sm font-medium">
                <ThumbsUp className="h-4 w-4 text-success" /> Plus points
              </p>
            ...
            <div className="glass rounded-2xl p-5">
              <p className="mb-3 flex items-center gap-2 text-sm font-medium">
                <ThumbsDown className="h-4 w-4 text-destructive" /> Drawbacks
              </p>
              <ul className="space-y-3">
            ...
            <div className="glass rounded-2xl p-5 lg:col-span-3">
              <p className="mb-3 flex items-center gap-2 text-sm font-medium">
                <ListChecks className="h-4 w-4 text-primary" /> 30-day action plan
              </p>
              <ol className="grid gap-2 md:grid-cols-2">
                {((resume.targeted_feedback as any).action_items ?? []).map(
                  (a: string, i: number) => (
                    <li
                      key={i}
                      className="flex gap-2 rounded-lg border border-border/60 p-3 text-sm"
                    >
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-primary/15 text-xs font-medium text-primary">
                        {i + 1}
                      </span>
                      <span>{a}</span>
                    </li>
          ...
          <p className="mt-6 text-sm text-muted-foreground">
            No role analysis yet. Fill the form above to generate one.
          </p>
        )}
      </div>
    </CandidateShell>
