            ...
            {SKILLS.map(s => (
              <div key={s.name}>
                <div className="flex items-center justify-between text-sm">
                  <span>{s.name}</span>
                  <span className="text-muted-foreground">{s.have} / {s.need}</span>
                </div>
                <div className="mt-1 relative">
                  <Progress value={s.have} className="h-2" />
                  <div className="absolute top-0 h-2 w-px bg-primary" style={{ left: `${s.need}%` }} />
                </div>
                <div className="mt-1 flex items-center justify-between text-[10px] uppercase tracking-wider">
                  <span className="text-muted-foreground">{s.priority === "strong" ? "Already above bar" : `Gap ${s.gap} pts`}</span>
                  <Badge variant={s.priority === "high" ? "destructive" : s.priority === "strong" ? "default" : "secondary"} className="rounded-full text-[9px]">{s.priority}</Badge>
                </div>
              </div>
            ))}
          </div>
            ...
            <ul className="mt-4 space-y-3 text-sm">
              {PLAN.map(p => (
                <li key={p.week} className="rounded-xl border border-border/40 bg-card/40 p-3">
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{p.week} · {p.hours}h</p>
