...
function CandidateDashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [profile, resumes, apps, sessions, learning] = await Promise.all([
        supabase
          .from("profiles")
          .select("full_name,phone,education,experience_level,avatar_url")
          .eq("id", user.user.id)
          .maybeSingle(),
        supabase
          .from("resumes")
          .select("id,ats_score,is_primary,title")
  ...
  const steps = [
    {
      done: !!p?.full_name && !!p?.experience_level,
      label: "Complete your profile",
      to: "/candidate/profile",
    },
    {
      done: (data?.resumes.length ?? 0) > 0,
    ...
    {
      done: (data?.sessions.length ?? 0) > 0,
      label: "Run a mock interview",
      to: "/candidate/interviews",
    },
    {
      done: (data?.learning.length ?? 0) > 0,
      label: "Start a learning plan",
  ...
  return (
    ...
    >
      <div className="grid gap-6 md:grid-cols-4">
        <Kpi
          label="Resume score"
          value={primary?.ats_score != null ? `${primary.ats_score}` : "—"}
          sub={primary ? primary.title : "Upload to compute"}
        />
        <Kpi
          label="Interview readiness"
          value={lastSession?.readiness_score != null ? `${lastSession.readiness_score}` : "—"}
          sub={lastSession ? lastSession.role_target : "Run a mock session"}
        />
        <Kpi
          label="Applications"
          value={`${data?.apps.length ?? 0}`}
          sub={`${data?.apps.filter((a) => a.stage === "interview").length ?? 0} in interview`}
        />
        <Kpi
          label="Learning items"
          value={`${inProgress}`}
          sub={`${(data?.learning.length ?? 0) - inProgress} completed`}
        />
      </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        ...
        >
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Build, edit, score and rewrite with AI.
          </p>
        ...
        >
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Adaptive mock interviews with scoring.
          </p>
          <span className="mt-4 inline-flex items-center gap-1 text-sm text-primary">
            Start a session{" "}
      ...
      {data?.sessions.length ? (
        <div id="interview-reports" className="glass mt-8 rounded-2xl p-6">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            AI-generated reports from every completed session. Download the PDF for offline review.
          </p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  {s.status === "completed" && (
                    <Button
                      ...
                      onClick={() =>
                        exportInterviewReport(s.id).catch((e) => toast.error(e.message ?? "Failed"))
                      }
                    >
