...
function TailorPage() {
  ...
  function run() {
    setLoading(true);
    setTimeout(() => { setResult(MOCK); setLoading(false); }, 900);
  }
    ...
    <CandidateShell eyebrow="Resume tailoring" title="Tailor to the role">
      <div className="grid gap-6 lg:grid-cols-[1fr_1.4fr]">
        <div className="glass rounded-2xl p-6">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Paste job description</p>
          <Textarea value={jd} onChange={(e) => setJd(e.target.value)} rows={16} className="mt-3 font-mono text-xs" />
        ...
        <div className="space-y-4">
          {!result && !loading && (
            <div className="glass rounded-2xl p-10 text-center text-muted-foreground">
              ...
              <p className="mt-3 text-sm">Run tailoring to see keyword coverage, rewritten bullets and a match score.</p>
            </div>
          )}
          {result && (
            <>
              <div className="glass rounded-2xl p-6">
                <div className="flex items-end justify-between">
                  <div>
                    ...
                    <p className="font-display text-5xl">{result.score}<span className="text-xl text-muted-foreground">/100</span></p>
                  </div>
              ...
              <div className="glass rounded-2xl p-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {result.keywords.map(k => (
                    <Badge key={k.term} variant={k.hit ? "default" : "outline"} className="rounded-full">
                      {k.hit && <Check className="mr-1 h-3 w-3" />}{k.term}
                    </Badge>
                  ))}
                </div>
              </div>
                ...
                <ul className="mt-3 space-y-3 text-sm">
                  {result.bullets.map((b, i) => (
                    <li key={i} className="rounded-xl border border-border/40 bg-card/40 p-3">
                      ...
                      <p className="mt-2 text-[10px] uppercase tracking-wider text-primary">after</p>
                      <p>{b.after}</p>
                    </li>
                  ))}
                </ul>
...
const MOCK = {
  score: 87,
  summary: "Your distributed-systems and Postgres experience map directly. Surface concrete throughput numbers and on-call wins to close the gap.",
  keywords: [
  ...
  bullets: [
    {
      ...
      after: "Designed an event-driven webhook ingestor in Python + Postgres LISTEN/NOTIFY processing 22K events/sec p95 with idempotent retries.",
    },
    {
      before: "Improved observability across services.",
      after: "Rolled out OpenTelemetry tracing across 14 services, cutting MTTR on payment incidents from 42m → 11m within a quarter.",
    },
  ],
};
