...
function RecruiterDashboard() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [jobs, apps, sessions, profile] = await Promise.all([
        ...
        supabase
          .from("applications")
          .select("id,stage,match_score,job_id,created_at,candidate_id,jobs!inner(recruiter_id)")
          .in("jobs.recruiter_id", teamIds),
        supabase
          .from("interview_sessions")
          .select("id,role_target,overall_score,created_at,job_id,jobs!inner(recruiter_id)")
          .in("jobs.recruiter_id", teamIds)
  ...
  const avgMatch = data?.apps.length
    ? Math.round(data.apps.reduce((s, a) => s + (a.match_score ?? 0), 0) / data.apps.length)
    : 0;

  return (
    <RecruiterShell
      eyebrow={data?.profile?.company_name ?? "Your team"}
      title={
        <span>
          <span className="text-gold">Welcome,</span>{" "}
          {data?.profile?.full_name?.split(" ")[0] ?? "Recruiter"}.
        </span>
      }
    >
      ...
      {!!data?.sessions.length && (
        <div className="glass mt-8 rounded-2xl p-6">
          <div className="flex items-center gap-3">
            ...
            <p className="font-medium">Recent interview replays</p>
          </div>
            ...
            {data.sessions.map((s) => (
              <li key={s.id} className="flex items-center justify-between py-3 text-sm">
                ...
                <div className="flex items-center gap-4">
                  <span className="font-display text-xl text-gold">{s.overall_score ?? "—"}</span>
                </div>
              </li>
            ))}
          </ul>
