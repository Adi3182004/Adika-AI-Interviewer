...
function SettingsPage() {
  ...
  const { data: myRoles } = useQuery({
    ...
    queryFn: async () => {
      ...
      return ((await supabase.from("user_roles").select("*").eq("user_id", u.user.id)).data ??
        []) as RoleRow[];
    },
  });
  ...
  const { data: team } = useQuery({
    ...
    queryFn: async () => {
      ...
      const profiles = ((
        await supabase
          .from("profiles")
          .select("id,email,full_name,company_name,primary_role")
          .in("id", ids)
      ).data ?? []) as ProfileRow[];
  ...
  return (
    <RecruiterShell
      ...
      title={
        <span>
          Recruiter <span className="text-gold">Command Settings</span>
        </span>
      }
    >
      ...
      {tab === "workspace" && (
        <div className="glass mt-6 max-w-3xl rounded-2xl p-8 space-y-4">
          <div className="flex items-center gap-3 border-b border-gold/20 pb-4">
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-gold-soft text-gold">
              <Building2 className="h-5 w-5" />
            </div>
            <div>
              <p className="font-display text-lg text-gold">Workspace identity</p>
              <p className="text-xs text-muted-foreground">
                Public profile for outbound messages and offer letters.
              </p>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <Textarea
              ...
              onChange={(e) => setForm({ ...form, hiring_goals: e.target.value })}
              className="mt-1"
              rows={3}
              placeholder="e.g. 10 engineers in Q1, focus on senior backend with payments domain"
            />
          </div>
          <Button
            onClick={save}
        ...
        <div className="glass mt-6 rounded-2xl p-8">
          <div className="flex items-center justify-between border-b border-gold/20 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {isAdmin
                    ? "You can promote, demote, and assign workspace roles."
                    : "Viewing only — admin permission required to edit roles."}
                </p>
              </div>
            </div>
            <Badge variant="outline" className="rounded-full border-gold/40 text-gold">
              {team?.profiles?.length ?? 0} members
            </Badge>
          </div>
                  ...
                  return (
                    <tr key={p.id} className="border-t border-border/20">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {userRoles.map((r) => (
                            <Badge
                              key={r.id}
                              variant="outline"
                              className={`rounded-full text-[10px] ${r.role === "admin" ? "border-gold text-gold" : ""}`}
                            >
                              {r.role}
                            </Badge>
                          ))}
                      ...
                      <td className="px-4 py-3 text-right">
                        <Select
                          value={primary}
                          onValueChange={(v) => changeRole(p.id, v as RoleRow["role"])}
                          disabled={!isAdmin}
                        >
                          <SelectTrigger className="ml-auto h-8 w-36 rounded-full text-xs">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
          ...
          {!isAdmin && (
            <p className="mt-4 flex items-center gap-2 rounded-xl border border-gold/30 bg-gold-soft/30 p-3 text-xs text-gold">
              <UserCog className="h-4 w-4" /> Ask a workspace admin to elevate your account to
              manage roles.
            </p>
          )}
        </div>
      ...
      {tab === "preferences" && (
        <div className="glass mt-6 max-w-3xl rounded-2xl p-8 space-y-2">
          <div className="flex items-center gap-3 border-b border-gold/20 pb-4">
            ...
            <div>
              <p className="font-display text-lg text-gold">AI hiring intelligence</p>
              <p className="text-xs text-muted-foreground">Tune how AI augments your pipeline.</p>
            </div>
          </div>
          {[
            {
              k: "ai_briefs",
              t: "Auto-generate recruiter briefs",
              d: "Summarize every new applicant with strengths, gaps, and signal score.",
            },
            {
              k: "email_digest",
              t: "Daily pipeline digest",
              d: "Get a morning email of moves, replies, and at-risk candidates.",
            },
            {
              k: "auto_match",
              t: "Auto-rank applicants",
              d: "Run match scoring the moment a new application is received.",
            },
            {
              k: "redact_pii",
              t: "Redact PII for blind review",
              d: "Hide names, photos, and addresses on the pipeline board.",
            },
          ].map((row) => (
            <div
              key={row.k}
              className="flex items-center justify-between gap-4 rounded-xl border border-border/30 bg-card/30 px-4 py-3"
            >
              <div>
          ...
          <Button
            ...
            className="mt-4 rounded-full bg-gold-soft text-gold border border-gold hover:bg-gold/20"
          >
...
function TeamInvitesPanel() {
  ...
  useEffect(() => {
    if (teamData?.team?.name) setNewName(teamData.team.name);
  }, [teamData?.team?.name]);
  ...
  async function doInvite() {
    ...
    setBusy(true);
    try {
      const { invite: inv } = await invite({
        data: { email: email.trim().toLowerCase(), role: "member" },
      });
  ...
  async function doRename() {
    ...
    try {
      await rename({ data: { name: newName.trim() } });
      qc.invalidateQueries({ queryKey: ["my-team"] });
  ...
  if (isLoading)
    return (
      <div className="glass mt-6 rounded-2xl p-12 text-center text-sm text-muted-foreground">
    ...
    <div className="glass mt-6 rounded-2xl p-8 space-y-6">
      <div className="flex items-center justify-between border-b border-gold/20 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Invite teammates so they can see the same jobs, applicants, and interview replays as
              you.
            </p>
          </div>
        </div>
        <Badge variant="outline" className="rounded-full border-gold/40 text-gold capitalize">
          {teamData?.role ?? "member"}
        </Badge>
      </div>
      ...
      {isOwner && (
        <div className="rounded-xl border border-gold/20 bg-gold-soft/30 p-4">
          <Label className="text-gold flex items-center gap-1">
            <Mail className="h-3 w-3" /> Invite a teammate by email
          </Label>
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            <Button
              ...
              className="rounded-full bg-gold text-background hover:bg-gold/90"
            >
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Create invite link"}
            </Button>
          </div>
          <p className="mt-2 text-[11px] text-muted-foreground">
            A single-use link is generated and copied to your clipboard. Invites expire after 14
            days.
          </p>
        </div>
      )}
                ...
                return (
                  <tr key={m.user_id} className="border-t border-border/20">
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        variant="outline"
                        className={`rounded-full text-[10px] capitalize ${m.role === "owner" ? "border-gold text-gold" : ""}`}
                      >
                        {m.role}
                      </Badge>
                    </td>
          ...
          <div className="space-y-2">
            {teamData!.invites
              ...
              .map((i) => (
                <div
                  key={i.id}
                  className="flex items-center gap-3 rounded-xl border border-border/30 bg-card/30 px-4 py-2 text-sm"
                >
                  ...
                  {isOwner && (
                    <Button
                      ...
                      className="text-xs text-rose-500/80 hover:text-rose-500"
                    >
                      <Trash2 className="h-3.5 w-3.5" />
                    </Button>
                  )}
