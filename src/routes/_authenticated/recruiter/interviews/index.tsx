...
function Replays() {
  ...
  const { data: sessions } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select(
          "*, jobs!inner(title,recruiter_id), profiles!interview_sessions_candidate_id_fkey(full_name,email)",
        )
  ...
  async function downloadReport() {
    ...
    setDownloading(true);
    try {
      await exportInterviewReport(openId);
    } catch (e: any) {
  ...
  return (
    <RecruiterShell
      eyebrow="Adaptive interviews"
      title={
        <span>
          Replayable <span className="text-gold">Sessions</span>
        </span>
      }
    >
        ...
        <div className="space-y-3">
          ...
          <div className="glass rounded-2xl p-2 max-h-[72vh] overflow-y-auto">
            {filteredSessions.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`block w-full rounded-xl p-3 text-left transition ${openId === s.id ? "bg-gold-soft ring-1 ring-gold/40" : "hover:bg-card/40"}`}
                >
                  <p className="font-medium truncate">{p?.full_name ?? "Candidate"}</p>
                  <p className="text-xs text-muted-foreground truncate">
                    {j?.title} · {s.role_target}
                    {co ? ` · ${co}` : ""}
                  </p>
            ...
            {!filteredSessions.length && (
              <p className="p-6 text-center text-sm text-muted-foreground">No replays yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {!open ? (
            <div className="glass rounded-2xl p-12 text-center text-sm text-muted-foreground">
              Pick a session on the left to replay the interview, see per-question analysis, and
            ...
            <>
              ...
              <div className="glass rounded-2xl p-5">
                <div className="flex flex-wrap items-start gap-3">
                    ...
                    <div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground">
                      {(open as any).company && (
                        <span className="flex items-center gap-1">
                          <Building2 className="h-3 w-3" /> {(open as any).company}
                        </span>
                      )}
                      {(open as any).experience_level && (
                        <span className="flex items-center gap-1 capitalize">
                          <GraduationCap className="h-3 w-3" /> {(open as any).experience_level}
                        </span>
                      )}
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="rounded-full border-gold/40 text-gold hover:bg-gold-soft"
                  >
                ...
                {open.summary && (
                  <div className="mt-4 rounded-xl border border-gold/20 bg-gold-soft/40 p-3 text-sm">
                    <p className="text-[10px] uppercase tracking-wider text-gold mb-1 flex items-center gap-1">
                      <Sparkles className="h-3 w-3" /> Teacher summary
                    </p>
                    {open.summary}
                  </div>
                )}
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {!!open.strengths?.length && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1">
                        <TrendingUp className="h-3 w-3 text-emerald-500" /> Strengths
                      </p>
                      <div className="flex flex-wrap gap-1">
                        {open.strengths.map((s, i) => (
                          <Badge
                            key={i}
                            variant="outline"
                            className="rounded-full border-emerald-500/30 text-emerald-500/90 text-[10px]"
                          >
                            {s}
                          </Badge>
                        ))}
                  ...
                  {!!open.gaps?.length && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1">
                        <AlertTriangle className="h-3 w-3 text-amber-500" /> Skills to build
                      </p>
                      <div className="flex flex-wrap gap-1">
                        {open.gaps.map((s, i) => (
                          <Badge
                            key={i}
                            variant="outline"
                            className="rounded-full border-amber-500/30 text-amber-500/90 text-[10px]"
                          >
                            {s}
                          </Badge>
                        ))}
                ...
                <div className="glass rounded-2xl p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setActiveQ((q) => Math.min(turns.length - 1, q + 1))}
                      disabled={activeQ >= turns.length - 1}
                    >
                      Next
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setActiveQ(turns.length - 1)}
                      disabled={activeQ >= turns.length - 1}
                    >
                      <SkipForward className="h-4 w-4" />
                    </Button>
                    <span className="ml-auto text-xs text-muted-foreground">
                      Question {activeQ + 1} of {turns.length}
                    </span>
                  </div>
                    ...
                    {turns.map((t, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`h-7 w-7 rounded-full border text-[10px] font-medium transition ${i === activeQ ? "bg-gold-soft" : ""} ${tone}`}
                        >
                          {i + 1}
                        </button>
                      );
              ...
              {activeTurn && (
                <div className="glass rounded-2xl p-5 space-y-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gold mb-1 flex items-center gap-1">
                      <Bot className="h-3 w-3" /> Question {activeQ + 1}
                    </p>
                  ...
                  {activeTurn.a ? (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1 flex items-center gap-1">
                        <User className="h-3 w-3" /> Candidate answer{" "}
                        {activeTurn.a.score != null && (
                          <span className="ml-auto text-gold">score {activeTurn.a.score}/100</span>
                        )}
                      </p>
                      <p className="text-sm whitespace-pre-wrap rounded-xl bg-card/40 p-3">
                        {activeTurn.a.content}
                      </p>
                      ...
                      {(activeSignals.feedback ||
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-2">
                          {activeSignals.feedback && (
                            <AnalysisBlock title="AI feedback" tone="default">
                              {activeSignals.feedback}
                            </AnalysisBlock>
                          )}
                          {!!activeSignals.what_was_good?.length && (
                            <AnalysisBlock title="What worked" tone="good">
                              <ul className="list-disc pl-4 space-y-0.5">
                                {activeSignals.what_was_good.map((x, i) => (
                                  <li key={i}>{x}</li>
                                ))}
                              </ul>
                            </AnalysisBlock>
                          ...
                          {!!activeSignals.what_to_improve?.length && (
                            <AnalysisBlock title="What to improve" tone="warn">
                              <ul className="list-disc pl-4 space-y-0.5">
                                {activeSignals.what_to_improve.map((x, i) => (
                                  <li key={i}>{x}</li>
                                ))}
                              </ul>
                            </AnalysisBlock>
                          ...
                          {activeSignals.ideal_answer_sketch && (
                            <AnalysisBlock title="Model answer sketch" tone="info">
                              {activeSignals.ideal_answer_sketch}
                            </AnalysisBlock>
                          )}
                        </div>
              ...
              {(open as any).job_description && (
                <div className="glass rounded-2xl p-4 text-xs">
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1">
                    <FileText className="h-3 w-3" /> Job description used
                  </p>
