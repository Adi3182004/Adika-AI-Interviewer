  ...
  const { data: apps } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("applications")
        .select(
          "*, profiles!applications_candidate_id_fkey(full_name,email,experience_level), resumes(id,title,content,parsed_skills,ats_score)",
        )
        .eq("job_id", id)
      ...
      for (const s of sessions ?? [])
        if (s.overall_score != null) scoreBy.set(s.candidate_id, s.overall_score);
      ...
      <div className="rounded-xl border border-border/50 bg-card/50 p-3 text-sm backdrop-blur">
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-start gap-2">
            <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-gold-soft text-[10px] font-medium text-gold">
              {index + 1}
            </span>
            <div className="min-w-0">
              ...
              <p className="truncate text-[10px] text-muted-foreground/80">
                {p?.experience_level ?? p?.email ?? ""}
              </p>
            </div>
          </div>
          <div className="flex shrink-0 flex-col items-end gap-1">
            <span
              className="rounded-full bg-gold-soft px-1.5 py-0.5 text-[10px] font-medium text-gold"
              title="Match score"
            >
              {a.match_score ?? "—"}%
            </span>
            {a.interview_score != null && (
              <span
                className="inline-flex items-center gap-0.5 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] text-emerald-400"
        ...
        <div className="mt-2 flex items-center justify-between gap-1">
          {stageDef?.prev ? (
            <Button
              size="sm"
              variant="ghost"
              className="h-6 px-2 text-[10px] text-muted-foreground hover:text-gold"
              onClick={() => move(a.id, stageDef.prev as Stage)}
              title="Move back"
            >
          ...
          {stageDef?.next ? (
            <Button
              size="sm"
              variant="ghost"
              className="h-6 px-2 text-[10px] text-gold hover:bg-gold-soft"
              onClick={() => move(a.id, stageDef.next as Stage)}
              title="Move forward"
            >
        ...
        {canAct && (
          <div className="mt-2 flex items-center gap-1 border-t border-border/40 pt-2">
            <Button
              size="sm"
              className="h-6 flex-1 bg-emerald-600 px-2 text-[10px] text-white hover:bg-emerald-700"
              onClick={() => move(a.id, "hired")}
            >
            ...
            <Button
              size="sm"
              variant="outline"
              className="h-6 flex-1 border-destructive/40 px-2 text-[10px] text-destructive hover:bg-destructive/10"
    ...
    return (
      <div className="glass min-w-[230px] flex-1 rounded-2xl p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-card/60 px-2 py-0.5 text-[10px] text-muted-foreground">
            {items.length}
          </span>
        </div>
        <div className="mt-3 space-y-2">
          {items.map((a, i) => (
            <Card key={a.id} a={a} stage={stageKey} index={i} />
          ))}
          {!items.length && (
            <p className="py-6 text-center text-xs text-muted-foreground italic">empty</p>
          )}
        </div>
      </div>
    );
    ...
    <RecruiterShell eyebrow={job?.title ?? "Pipeline"}>
      ...
      <Tabs defaultValue="board" className="w-full">
        <TabsList className="glass rounded-2xl p-1">
          <TabsTrigger
            value="board"
            className="rounded-xl data-[state=active]:bg-gold-soft data-[state=active]:text-gold"
          >
          ...
          <TabsTrigger
            ...
            className="rounded-xl data-[state=active]:bg-gold-soft data-[state=active]:text-gold"
          >
            <Trophy className="mr-2 h-4 w-4" /> New Employees{" "}
            <span className="ml-2 text-[10px] opacity-70">{hired.length}</span>
          </TabsTrigger>
          <TabsTrigger
            value="archive"
            className="rounded-xl data-[state=active]:bg-gold-soft data-[state=active]:text-gold"
          >
            ...
            <span className="ml-2 text-[10px] opacity-70">{rejected.length}</span>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="board" className="mt-6">
          ...
          <p className="mt-3 text-[11px] text-muted-foreground">
            Tip: Use the arrows on each card to advance or step back a stage. Approve / Reject
            buttons appear automatically at the <span className="text-gold">Interview</span> and{" "}
            <span className="text-gold">Offer</span> stages.
          </p>
        </TabsContent>
        ...
        <TabsContent value="archive" className="mt-6">
          <ArchiveTab
            items={rejected}
            jobTitle={job?.title}
            onRestore={(appId) => move(appId, "interview")}
            onDelete={remove}
...
function HiredTab({
  ...
  items: any[];
  jobTitle?: string;
  setResumeOpen: (v: any) => void;
}) {
  if (!items.length)
    return (
      <div className="glass rounded-2xl p-12 text-center text-sm text-muted-foreground">
        No new employees yet — approve someone at Interview or Offer to move them here.
      </div>
    );
  return (
          ...
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-emerald-400 flex items-center gap-1">
                <UserCheck className="h-3 w-3" /> Accepted · #{i + 1}
              </p>
            ...
            <div className="text-right">
              ...
              {a.interview_score != null && (
                <p className="text-[10px] text-emerald-400">Interview {a.interview_score}</p>
              )}
            </div>
          </div>
          {a.resumes && (
            <Button
              ...
              onClick={() => setResumeOpen({ name: a.profiles?.full_name, resume: a.resumes })}
            >
      ...
      {items.map((a, i) => (
        <div key={a.id} className="glass rounded-2xl p-5 opacity-90">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-destructive flex items-center gap-1">
                <Archive className="h-3 w-3" /> Archived · #{i + 1}
              </p>
              <p className="mt-1 font-display text-lg">{a.profiles?.full_name ?? "Candidate"}</p>
              <p className="text-xs text-muted-foreground">{jobTitle ?? "—"}</p>
            </div>
          ...
          <div className="mt-3 flex gap-2">
            <Button
              size="sm"
              className="h-7 flex-1 bg-gold text-background text-[11px] hover:bg-gold/90"
            ...
            <Button
              ...
              onClick={() => {
                if (confirm("Delete permanently?")) onDelete(a.id);
              }}
            >
...
function CompareTab({ apps }: { apps: any[] }) {
  ...
  const dims: { key: string; label: string; get: (a: any) => number | string }[] = [
    ...
    { key: "skills", label: "Skills tracked", get: (a) => a.resumes?.parsed_skills?.length ?? 0 },
    {
      ...
      get: (a) => a.profiles?.experience_level ?? "—",
    },
    ...
    <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
      <div className="glass rounded-2xl p-4">
        <p className="text-xs uppercase tracking-[0.25em] text-gold/80 flex items-center gap-2">
          <Users className="h-3 w-3" /> Select up to 4
        </p>
      ...
      <div className="glass overflow-hidden rounded-2xl">
        {selected.length === 0 ? (
          <div className="p-8 text-center text-sm text-muted-foreground">
            Select candidates on the left to build a side-by-side comparison matrix.
          </div>
        ) : (
                    ...
                    <tr key={d.key} className="border-b border-border/60">
                      ...
                      {selected.map((a) => {
                        ...
                        return (
                          <td
                            key={a.id}
                            className={`px-4 py-3 text-right font-display ${isBest ? "text-gold" : "text-foreground"}`}
                          >
                            {v}
                            {isBest && <Sparkles className="ml-1 inline h-3 w-3" />}
                          </td>
                        );
                      })}
                    </tr>
